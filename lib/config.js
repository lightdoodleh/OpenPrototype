...
const DEFAULTS = {
  ...
  // lanAddress 由引用 openprototype 的业务配置，可按 roots 中的端分别配置。
  // 旧版字符串写法继续兼容，表示该产品所有端共用一个地址。
  products: [{ id: 'demo', roots: ['pc'], lanAddress: { pc: '' } }]
};
...
function resolveLanAddress(product, surface) {
  const configured = product && product.lanAddress;
  if (typeof configured === 'string') return configured.trim();
  if (!configured || Array.isArray(configured) || typeof configured !== 'object') return '';

  const root = String(surface || '').trim();
  if (!root || !Array.isArray(product.roots) || !product.roots.includes(root)) return '';
  return typeof configured[root] === 'string' ? configured[root].trim() : '';
}
...
function loadConfig(startDir) {
  ...
  return {
    ...
    host: merged.host,
    service: {
      autoInstall: !merged.service || merged.service.autoInstall !== false
    },
    opencode: {
...
module.exports = { loadConfig, detectOpenCodeBin, findProjectRoot, resolveLanAddress, DEFAULTS, CONFIG_FILENAME };
